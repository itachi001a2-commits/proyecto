...
} className = "p-2 bg-gray-100 rounded-lg" > <X size={ 16 } className = "text-gray-600" /> </button>
  </div>
                  ) : (
    ...
    <div className="px-3 pb-1.5 pt-1 bg-white shrink-0 transition-all" onTouchStart = { handleTouchStart } onTouchEnd = { handleTouchEnd } >
      <div className="grid grid-cols-4 gap-1" >
        <button onClick={ () => handleKeypad("1") } className = "keypad-btn" > 1 </button>
          < button onClick = {() => handleKeypad("2")
} className = "keypad-btn" > 2 </button>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm max-h-[80%] flex flex-col" >
        <div className="flex items-center justify-between p-4 border-b border-gray-100" >
          <h3 className="font-bold text-lg text-gray-900" > Seleccionar Loterias </h3>
            < button onClick = {() => setShowLotteryModal(false)
        ...
        { openLotteries.length > 0 && <div className="text-[10px] font-bold text-green-700 mb-1"> ABIERTAS AHORA</ div >}
{
  openLotteries.map((lottery) => {
    ...
    return (
      <button key= { lottery.id } onClick = {() => { setSelectedLotteries((prev) => isSelected ? prev.filter((l) => l.id !== lottery.id) : [...prev, lottery]); }
  } className = {`w-full flex items-center gap-3 p-3 rounded-xl border transition-all text-left ${isSelected ? "bg-green-50 border-green-400" : "bg-white border-gray-200 hover:border-green-300"}`}>
...
} className = "w-full mt-2 py-2.5 rounded-xl bg-gray-100 text-gray-700 font-bold text-sm hover:bg-gray-200" > Cerrar </button>
  </div>
  </div>
      )}
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm max-h-[85%] flex flex-col" >
        <div className="flex items-center justify-between p-4 border-b border-gray-100" >
          <h3 className="font-bold text-lg text-gray-900" > Jugada Rapida </h3>
            < button onClick = {() => { setShowQuickPlay(false); setQuickType(null); }
} className = "p-1.5 hover:bg-gray-100 rounded-lg" > <X size={ 20 } className = "text-gray-500" /> </button>
  </div>
      ...
      <Hash size={ 24 } className = "text-green-600" />
        <span className="text-sm font-bold text-green-700" > PAR </span>
          < span className = "text-[10px] text-green-500" > 11, 22, 33...00 </span>
            ...
            < button onClick = {() => setQuickType("decimal")
} className = "flex flex-col items-center gap-2 p-4 bg-blue-50 rounded-xl border border-blue-200 hover:bg-blue-100 transition-all" >
